...
class RecordReservationLine(models.Model):                   
    ...
    name_reservation_line = fields.Char("Description", required=True)
    record = fields.Many2one("record.record", "Record")
    artist_id = fields.Many2one(
        "record.artist", "Artist"
    )
    price = fields.Float("Price Record")
    reservation_unit = fields.Integer("Reserved Units", default = "1")
    client = fields.Many2one("res.partner", "Client")
    total_price = fields.Float("Total Price", compute="_compute_total_reservation", store=True)
